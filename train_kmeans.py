...
def train_kmeans(I, O,m=500,gw=1.):
    """
    K-means based parameter selection for RBF. 
    candidate centers,c_i, are selected using k-means clustering. 
    Corresponding widths, sigma_i, are assigned the average 
    distortion in their cluster. 
    """
    m = min(O.shape[0],m)
    I_whitened = whiten(I)
    # We need to randomly initialize centers
    center_idx = np.arange(O.shape[0])
    np.random.shuffle(center_idx)
    center_idx = center_idx[:m]
    

    codebook = I_whitened[center_idx,:]
    codebook,distortion = kmeans(I_whitened,codebook)
    kmeans_idx,kmeans_distances = vq(I_whitened,codebook)
    sigma = np.zeros((m,1))
    for i in range(m):
